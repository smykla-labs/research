...
- Chained from agent-manager when `slash_command: yes` (command name already specified)
...
**Dispatch mode** (chained from agent-manager):
...
**For Dispatch mode** (chained from agent-manager) — Skip this phase when command name and location are already provided in the input. Proceed directly to Phase 3.
...
<input>Create command for agent-manager agent</input>
<output>
...
1. **Invoke agent-manager** with the Task tool
   - Include full user request: `$ARGUMENTS`
   - State detected mode explicitly
2. **Parse status block** from output:
   - `STATUS: NEEDS_INPUT` → Parse questions, use `AskUserQuestion` tool, resume with `ANSWERS:`
   - `STATUS: COMPLETED` → Report success to user
   - `STATUS: READY_FOR_COMMAND` → Invoke command-manager for slash command
3. **Repeat** until final `STATUS: COMPLETED`
...
<input>Create command for worktree-manager agent</input>
<output>
...
<example type="bad">
<input>MODIFY .claude/commands/git/clean-gone.md</input>
<why_bad>
...
<example type="bad">
<input>MODIFY .claude/commands/manager/plan.md (agent outputs prose instead of STATUS block)</input>
<why_bad>
