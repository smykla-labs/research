...
from .models import ScreenshotError, WindowInfo
...
def _find_sc_window(window_id: int, timeout: float = 5.0):
    ...
    Raises:
        ScreenshotError: If shareable content fetch fails.
    """
    ...
    if context.error:
        raise ScreenshotError(f"Failed to get shareable content: {context.error}")
...
def capture_with_screencapturekit(
    ...
    Args:
        window: Window info (must include window_id).
    ...
    Raises:
        ScreenshotError: If capture fails or window not found.
    """
    if not is_screencapturekit_available():
        raise ScreenshotError("ScreenCaptureKit requires macOS 12.3 or later")
    ...
    sc_window = _find_sc_window(window.window_id, timeout=timeout)
    if sc_window is None:
        raise ScreenshotError(
            f"Window {window.window_id} not found in ScreenCaptureKit. "
    ...
    if context.error:
        raise ScreenshotError(f"Screenshot capture failed: {context.error}")

    if context.image is None:
        raise ScreenshotError("Screenshot capture timed out or returned no image")
    ...
    if dest is None:
        raise ScreenshotError(f"Failed to create image destination: {output_path}")
